...
mongo_client = MongoClient('mongodb://admin:adminpassword@192.168.1.12:27017/')
...
class Coche:
    ...
    def get_all():
        ...
        conn.close()
        return [
            Coche(marca=row[1], modelo=row[2], precio=row[3], anyo=row[4], tipo=row[5], autonomia=row[6], en_oferta=bool(row[7]), 
    ...
    def search(marca=None, modelo=None, precio_max=None, anyo=None, tipo=None, autonomia_min=None, tiempo_carga_max=None, capacidad_bateria=None, tipo_hibrido=None, consumo_mixto_max=None):
        ...
        return [
            Coche(
                marca=row[1], modelo=row[2], precio=row[3], anyo=row[4], tipo=row[5], autonomia=row[6], 
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if "user_id" not in session:
            flash("Inicia sesión para acceder.")  # Mostrar mensaje (extra)
...
def anuncios():
    ...
    coches = [
        Coche(id=row[0], marca=row[1], modelo=row[2], precio=row[3], anyo=row[4], tipo=row[5].lower(), autonomia=row[6], 
              en_oferta=bool(row[7]), tiempo_carga=row[8], capacidad_bateria=row[9], 
    ...
    return render_template("anuncios.html", anuncios=coches, comentarios_por_anuncio=comentarios_por_anuncio, historial_por_coche=historial_por_coche)
...
def editar_anuncio(coche_id):
    ...
    if request.method == "POST":
        ...
        # 🔹 Verificar que el precio es válido antes de guardarlo en MongoDB
        if nuevo_precio and nuevo_precio.isdigit():
            nuevo_precio = int(nuevo_precio)

            # 🔹 Actualizar los datos del coche en MariaDB, incluyendo los nuevos campos
            conn = get_mariadb_connection()
            with conn.cursor() as cursor:
                cursor.execute("""
                    UPDATE coches 
...
def borrar_anuncio(coche_id):
    ...
    # Verificar qué coches ha comprado el usuario
    usuario_id = session.get("user_id")
    usuario_compro_coche = {}
    if usuario_id:
        conn = get_mariadb_connection()
        with conn.cursor() as cursor:
            cursor.execute("SELECT coche_id FROM transacciones WHERE comprador_id = %s", (usuario_id,))
            compras = cursor.fetchall()
        conn.close()
        usuario_compro_coche = {compra[0]: True for compra in compras}

    return render_template("anuncios.html",
                        anuncios=coches,
                        comentarios_por_anuncio=comentarios_por_anuncio,
                        usuario_compro_coche=usuario_compro_coche)
...
def publicar():
    if request.method == "POST":
        ...
        # Capturar los nuevos datos SOLO si el tipo es híbrido o eléctrico
...
def favoritos():
    ...
    favorito = favoritos_col.find_one({'user_id': str(user_id)})  # Asegurarse de que 'user_id' esté bien formateado
        ...
        with conn.cursor() as cursor:
            ...
            coches_favoritos = [
                Coche(marca=row[1], modelo=row[2], precio=row[3], anyo=row[4],
                      tipo=row[5], autonomia=row[6], en_oferta=bool(row[7]),
    ...
    return render_template('favoritos.html', anuncios=coches_favoritos)
...
def ventas():
    ...
    for coche in coches_vendidos:
        ofertas = list(ofertas_col.find({"coche_id": int(coche.id)}))  # Convertir coche.id a int
        for oferta in ofertas:
            oferta["_id"] = str(oferta["_id"])  # Convertir ObjectId a string
        ofertas_por_coche[coche.id] = ofertas

    return render_template('ventas.html', anuncios=coches_vendidos, ofertas_por_coche=ofertas_por_coche)
...
@login_required
def mis_compras():
    usuario_id = session.get("user_id")  # Obtener el usuario desde la sesión

    # Obtener las compras del usuario desde MariaDB, incluyendo nuevos atributos
    conn = get_mariadb_connection()
    with conn.cursor() as cursor:
        cursor.execute("""
    ...
    # Obtener los comentarios del usuario sobre coches comprados
    comentarios_por_coche = {}
    for compra in coches_comprados:
        resultado = comentarios_col.find_one({"coche_id": compra["coche_id"]})  # ✅ Buscar el coche en MongoDB

        # Extraer correctamente los comentarios
        if resultado and "comentarios" in resultado:
            comentarios_por_coche[compra["coche_id"]] = [
                {"usuario_id": c["usuario_id"], "texto": c["comentario"], "fecha": c["fecha"]}
                for c in resultado["comentarios"]
            ]
        else:
            comentarios_por_coche[compra["coche_id"]] = []

    return render_template("mis_compras.html", compras=coches_comprados, comentarios_por_coche=comentarios_por_coche)
